...
var __uzdbm_1 = "625cf9ff-dc4d-4ef5-9c50-f088bc7c8e01";
var __uzdbm_2 = "MzMxNjZiZWUtZGs3dS00ZjA4LTlmZGItYTM0MTA3MjYwMDdjJDE3Ny4xMDcuMjkuMTAw";
var __uzdbm_3 = "";
var __uzdbm_4 = "false";
var __uzdbm_5 = "";
var __uzdbm_6 = "";
...
(function (win, doc, tag, src, cid, confName) {
    // Inicializa o objeto conector se não existir
    win["SSJSConnectorObj"] = win["SSJSConnectorObj"] || {
        ss_cid: cid,
        domain_info: "auto"
    };

    // Função auxiliar para definir configurações
    win[confName] = function (key, value) {
        win["SSJSConnectorObj"][key] = value;
    };

    // Criação dinâmica do elemento script
    var scriptElem = doc.createElement(tag);
    scriptElem.async = true;

    // Ajuste de compatibilidade para Internet Explorer (MSIE ou Trident)
    if (navigator.userAgent.indexOf('MSIE') !== -1 || navigator.appVersion.indexOf('Trident/') > -1) {
        src = src.replace("/advanced/", "/advanced/ie/");
    }

    scriptElem.src = src;

    // Insere o script no DOM antes do primeiro script encontrado
...
// Definições de parâmetros do conector
ssConf("c1", "https://suap.ifpi.edu.br");
ssConf("c3", "c99a4269-161c-4242-a3f0-28d44fa6ce24");
ssConf("au", "suap.ifpi.edu.br");
  ...
  if(pl.state.type === 'main'){
    ...
    if(res === 'TARGET'){
      ...
      movePlayerWithAnimation(pl, target, ()=> {
        ...
        if(pl.state.index === 4 && lastDice === 5){
            immunity[pl.id] = 2; // Duas rodadas imune 
            // pl.skipTurns = (pl.skipTurns || 0) + 1;
            logMsg(`${pl.name} tirou 5 e caiu na casa 05 → IMUNIDADE por 2 rodadas!.`);
            updateRuleDisplay(`${pl.name} está imune por 2 rodada (casa 05).`);
        }

        // house 9 index 8 => Erro Fatal => back to OFF (play alert) e  house 19 index 18 => Erro Fatal -> back to OFF
        if(pl.state.index === 8 || pl.state.index === 18){
          if(audioCtx.state === 'suspended') audioCtx.resume();
          playAlertA(); // alert sound for 9/19
          playLossSound();
          logMsg(`${pl.name} caiu na casa ${lastDice} (Erro Fatal!) e volta para CASA OFF.`);
        ...
        if(pl.state.index === 12){
          ...
          updateRuleDisplay(`Jogador ${pl.name} perderá 1 rodada (casa 13).`);
        }
